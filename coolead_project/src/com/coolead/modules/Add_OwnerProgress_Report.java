...
public class Add_OwnerProgress_Report {
	public static void add(WebDriver driver ) throws Exception{
		...
		Thread.sleep(1000);
		new Select(addOwnerProgressPage.report_subject_select()).selectByVisibleText(Constant.subject_name);
		addOwnerProgressPage.report_reviewer_select().click();
		addOwnerProgressPage.report_reviewer_select_confirm().click();
		addOwnerProgressPage.report_month_amount().sendKeys(Constant.amount);
		addOwnerProgressPage.report_total_amount().sendKeys(Constant.amount);
		addOwnerProgressPage.report_owner_progress_comment().sendKeys(Constant.owner_progress_report_comment);
		Thread.sleep(1000);
		addOwnerProgressPage.report_progress_status().click();
		addOwnerProgressPage.report_owner_progress_submit().click();
		addOwnerProgressPage.report_submit_confirm().click();
	}
